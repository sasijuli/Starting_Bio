# VMD Basics References

These references support installation, visualization, atom selections, Tcl scripting, measurements, trajectory viewing, and introductory analysis.

## Official VMD Documentation

- VMD main page: https://www.ks.uiuc.edu/Research/vmd/
- Current VMD User's Guide: https://www.ks.uiuc.edu/Research/vmd/current/ug.pdf
- VMD tutorials page: https://www.ks.uiuc.edu/Training/Tutorials/
- Official Using VMD tutorial, HTML: https://www.ks.uiuc.edu/Training/Tutorials/vmd/tutorial-html/index.html
- Official Using VMD tutorial, PDF: https://www.ks.uiuc.edu/Training/Tutorials/vmd/vmd-tutorial.pdf
- VMD Images and Movies tutorial: https://www.ks.uiuc.edu/Training/Tutorials/vmd-imgmv/imgmv/tutorial-html/node3.html

## Command References

- `atomselect` command reference: https://www.ks.uiuc.edu/Research/vmd/vmd-1.7.1/ug/node108.html
- Molecular analysis with `atomselect`: https://www.ks.uiuc.edu/Research/vmd/vmd-1.7.1/ug/node181.html
- `molinfo` command reference: https://www.ks.uiuc.edu/Research/vmd/vmd-1.8.7/ug/node137.html
- `measure` command reference, VMD 1.8.5: https://www.ks.uiuc.edu/Research/vmd/vmd-1.8.5/ug/node124.html
- `measure` command reference, VMD 1.8.6: https://www.ks.uiuc.edu/Research/vmd/vmd-1.8.6/ug/node124.html

The command pages above document older VMD releases but remain useful for the commands cited by this tutorial. Check the current User's Guide when behavior appears different.

## Supplementary Reference

- Compchems guide to distances, angles, and dihedrals in VMD: https://www.compchems.com/vmd-measurements-analyze-distances-and-angles-with-vmd/#tracking-distance-during-the-simulation

## Video Tutorials

### VMD Basics And Molecular Visualization

- Molecular visualization with VMD: https://www.youtube.com/watch?v=_skmrS6X4Ys

### Molecular Modeling With VMD

- Water Atomic Modeling with VMD: https://www.youtube.com/watch?v=wWuV6mW239I

### Mixed Molecular Simulation Playlists

- Mohamed shehata: https://www.youtube.com/@Mohamedshehata/playlists

## Related Repository Resources

- [VMD Basics overview](README.md)
- [VMD module references](../references.md)
- [VMD supplementary link summaries](../link_summaries.md)
- [Complete repository references](../../references.md)
